    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
            <div class="flex items-center space-x-2 text-xs font-mono text-[var(--text-muted)] mb-1">
                ...
                <span>/</span>
                <span class="text-[var(--text-primary)] font-bold">Edit Question</span>
            </div>
...
@section('content')
    <div class="max-w-4xl mx-auto" x-data="{ questionType: '{{ $question->type ?? 'multiple_choice' }}' }">
        <form action="{{ route('admin.quizzes.update-question', [$quiz, $question]) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="bg-[var(--glass-bg)] backdrop-blur-sm rounded-2xl p-6 sm:p-8 border border-[var(--border-color)] shadow-lg space-y-6">
                ...
                <div class="flex items-center justify-between border-b border-[var(--border-color)] pb-4">
                    <div>
                        <h3 class="text-base font-bold text-[var(--text-primary)]">Update Question Parameters & Answer Key</h3>
                        <p class="text-xs text-[var(--text-secondary)]">Modify question prompt, point weight, answer choices or explanation</p>
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row gap-4">
                        <div class="flex-1">
                            ...
                            <textarea name="question_text" rows="3" required
                                      class="w-full px-4 py-3 text-sm bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl focus:ring-2 focus:ring-blue-500 shadow-sm text-[var(--text-primary)] placeholder:text-[var(--text-muted)]">{{ old('question_text', $question->question_text) }}</textarea>
                        ...
                        <div class="w-full sm:w-36">
                            ...
                            <input type="number" name="points" value="{{ old('points', $question->points) }}" min="1" max="100" required
                ...
                <div x-show="questionType === 'essay'" class="p-4 rounded-2xl bg-purple-500/10 border border-purple-500/30 text-purple-800 dark:text-purple-300 space-y-2">
                ...
                <div x-show="questionType === 'multiple_choice'" class="space-y-4 pt-2">
                    ...
                    <div class="space-y-3">
                        ...
                        @for($i = 0; $i < 4; $i++)
                            @php
                                $opt = $optionsList[$i] ?? null;
                                $isCorrect = $opt ? $opt->is_correct : ($i == 0);
                                $optVal = $opt ? $opt->option_text : '';
                            @endphp
                            <div class="flex items-center space-x-3 p-2.5 rounded-xl bg-[var(--bg-card)] border border-[var(--border-color)] focus-within:border-blue-500 transition">
                                <label class="flex items-center justify-center p-2 cursor-pointer" title="Mark as correct answer">
                                    <input type="radio" name="correct_option" value="{{ $i }}" {{ $isCorrect ? 'checked' : '' }}
                                           class="w-4 h-4 text-emerald-600 focus:ring-emerald-500">
                                </label>
                                <span class="w-6 font-mono font-bold text-xs text-[var(--text-muted)] text-center">{{ chr(65 + $i) }}</span>
                                <input type="text" name="options[]" value="{{ old('options.' . $i, $optVal) }}" placeholder="Enter choice text..."
                ...
                <div x-show="questionType === 'true_false'" class="space-y-4 pt-2" style="display: none;">
                    @php
                        ...
                        $isTrueCorrect = $trueOption ? $trueOption->is_correct : true;
                    @endphp
                    <label class="block text-xs font-bold uppercase tracking-wider text-[var(--text-secondary)]">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <label class="flex items-center space-x-3 p-4 rounded-xl bg-[var(--bg-card)] border border-[var(--border-color)] cursor-pointer hover:border-emerald-500 transition">
                            <input type="radio" name="correct_tf" value="true" {{ $isTrueCorrect ? 'checked' : '' }} class="w-5 h-5 text-emerald-600 focus:ring-emerald-500">
                            <div>
                        ...
                        <label class="flex items-center space-x-3 p-4 rounded-xl bg-[var(--bg-card)] border border-[var(--border-color)] cursor-pointer hover:border-red-500 transition">
                            <input type="radio" name="correct_tf" value="false" {{ !$isTrueCorrect ? 'checked' : '' }} class="w-5 h-5 text-red-600 focus:ring-red-500">
                            <div>
                ...
                <div class="pt-2">
                    ...
                    <textarea name="explanation" rows="2"
                              ...
                              class="w-full px-4 py-3 text-sm bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl focus:ring-2 focus:ring-blue-500 shadow-sm text-[var(--text-primary)] placeholder:text-[var(--text-muted)]">{{ old('explanation', $question->explanation) }}</textarea>
                </div>
