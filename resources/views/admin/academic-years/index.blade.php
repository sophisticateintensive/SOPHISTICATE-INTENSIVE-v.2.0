...
@section('header')
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
            ...
            <p class="text-sm text-[var(--text-secondary)] mt-1">Manage school academic sessions, date spans and associated terms</p>
        </div>
...
@section('content')
    ...
    <div class="relative overflow-hidden rounded-3xl bg-gradient-to-br from-blue-600 via-indigo-600 to-purple-700 p-6 sm:p-8 mb-8 shadow-2xl">
        ...
        <div class="relative z-10 flex flex-col sm:flex-row items-center justify-between gap-4"
             x-data="{ counters: { total: 0, terms: 0, enrollments: 0 }, init() {
                const targets = { total: {{ $totalYears }}, terms: {{ $totalTermsCount }}, enrollments: {{ $activeEnrollments }} };
            ...
            <div>
                <div class="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm px-4 py-1.5 rounded-full border border-white/20 text-sm font-medium text-white mb-3">
                    ...
                    Active Session: {{ $activeYear?->year_name ?? 'No active session' }}
                </div>
                <h1 class="text-2xl sm:text-3xl font-bold text-white">Academic Calendar Master</h1>
                <p class="text-blue-100/80 mt-1">Manage institutional academic years, date schedules and semester terms</p>
            </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
                <div class="bg-white/10 backdrop-blur-sm rounded-xl px-4 py-3 text-center border border-white/10 hover:scale-105 transition-transform">
                    ...
                    <div class="text-xs text-blue-200 uppercase tracking-wider">Academic Years</div>
                </div>
                <div class="bg-white/10 backdrop-blur-sm rounded-xl px-4 py-3 text-center border border-white/10 hover:scale-105 transition-transform">
                    <div class="text-2xl font-bold text-emerald-300" x-text="counters.terms"></div>
                    <div class="text-xs text-blue-200 uppercase tracking-wider">Semesters / Terms</div>
                </div>
                <div class="bg-white/10 backdrop-blur-sm rounded-xl px-4 py-3 text-center border border-white/10 hover:scale-105 transition-transform">
                    <div class="text-2xl font-bold text-purple-300" x-text="counters.enrollments"></div>
                    <div class="text-xs text-blue-200 uppercase tracking-wider">Active Enrollments</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <form method="GET" action="{{ route('admin.academic-years.index') }}" id="filterForm" class="mb-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                ...
                <div class="lg:col-span-2">
                    <div class="relative">
                        ...
                        <input type="text" name="search" id="searchInput" value="{{ request('search') }}"
                               placeholder="Search academic year name..."
                        ...
                        @if(request('search'))
                            <a href="{{ route('admin.academic-years.index') }}" class="absolute inset-y-0 right-0 pr-3 flex items-center text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors">
                ...
                <div class="relative">
                    <select name="status" id="statusFilter"
                        ...
                        <option value="active" {{ request('status') == 'active' ? 'selected' : '' }}>Current Active</option>
                    </select>
                ...
                <div class="flex space-x-2">
                    ...
                    <a href="{{ route('admin.academic-years.index') }}" class="px-3 py-3 text-sm bg-[var(--bg-card)] text-[var(--text-primary)] rounded-xl border border-[var(--border-color)] hover:bg-[var(--glass-bg)] hover:scale-105 transition-all duration-200 flex items-center">
                        <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <!-- ========== ACADEMIC YEARS TABLE (STUDENT MANAGEMENT DESIGN) ========== -->
    <div class="bg-[var(--glass-bg)] backdrop-blur-sm rounded-2xl shadow-lg border border-[var(--border-color)] overflow-hidden">
        <div class="bg-gradient-to-r from-blue-600 via-blue-500 to-indigo-600 px-6 py-4 flex items-center justify-between">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <h3 class="text-xl font-bold text-white tracking-wide">All Academic Sessions</h3>
                    <p class="text-xs text-blue-200 mt-0.5">Manage session terms, start & end dates</p>
                </div>
            </div>
        ...
        <div class="p-6">
                    ...
                    <table class="min-w-full divide-y divide-[var(--border-color)]">
                        <thead>
                            <tr>
                                ...
                                <th class="px-6 py-4 text-left text-xs font-semibold text-[var(--text-secondary)] uppercase tracking-wider">Start Date</th>
                                <th class="px-6 py-4 text-left text-xs font-semibold text-[var(--text-secondary)] uppercase tracking-wider">End Date</th>
                                <th class="px-6 py-4 text-left text-xs font-semibold text-[var(--text-secondary)] uppercase tracking-wider">Associated Terms</th>
                        ...
                        <tbody class="divide-y divide-[var(--border-color)]">
                            @foreach($academicYears as $year)
                                <tr class="hover:bg-[var(--accent-soft)] transition-colors duration-200 {{ $year->is_current ? 'bg-emerald-500/5' : '' }}">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        @else
                                            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold font-mono bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400">
                                                Archive
                                            </span>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <p class="text-sm font-bold text-[var(--text-primary)]">{{ $year->year_name }}</p>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-xs text-[var(--text-secondary)] font-mono">
                                        {{ $year->start_date ? \Carbon\Carbon::parse($year->start_date)->format('M d, Y') : 'N/A' }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-xs text-[var(--text-secondary)] font-mono">
                                        {{ $year->end_date ? \Carbon\Carbon::parse($year->end_date)->format('M d, Y') : 'N/A' }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-bold font-mono bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300">
                                            {{ $year->terms->count() }} Term(s)
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right">
                                        <div class="flex items-center justify-end space-x-2">
                                            <a href="{{ route('admin.academic-years.show', $year) }}" class="p-2 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 rounded-lg hover:bg-indigo-100 dark:hover:bg-indigo-900/30 transition-all" title="View">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                                </svg>
                                            </a>
                                            <a href="{{ route('admin.academic-years.edit', $year) }}" class="p-2 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900/30 transition-all" title="Edit">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                                </svg>
                                            </a>
                                            <form action="{{ route('admin.academic-years.destroy', $year) }}" method="POST" id="delete-year-{{ $year->id }}" class="inline">
                                                @csrf @method('DELETE')
                                                <button type="button" onclick="openDeleteModal(
                                                    'Delete Academic Year?',
                                                    'You are about to delete {{ addslashes($year->year_name) }}. This action cannot be undone.',
                                                    document.getElementById('delete-year-{{ $year->id }}')
            ...
            @else
                <div class="text-center py-16">
                    ...
                    <h4 class="text-lg font-semibold text-[var(--text-primary)] mb-2">No Academic Years Found</h4>
                    <p class="text-[var(--text-secondary)] mb-4">Get started by creating your first academic session</p>
                    <a href="{{ route('admin.academic-years.create') }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-lg hover:shadow-lg transition-all duration-200">
